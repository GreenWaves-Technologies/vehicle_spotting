...
static void RunNetwork()
{
  ...
  gap_cl_resethwtimer();
#endif
  mbv1_grayscaleCNN(imgin_signed, ResOut);
...
/*------------------- Config Buffer for LCD Display ----------------*/
	buffer.data = imgin_unsigned;//+AT_IN_WIDTH*2+2;
	buffer.stride = 0;

	// WIth Himax, propertly configure the buffer to skip boarder pixels
	pi_buffer_init(&buffer, PI_BUFFER_TYPE_L2, imgin_unsigned);//+AT_IN_WIDTH*2+2);
	pi_buffer_set_stride(&buffer, 0);
	pi_buffer_set_format(&buffer, AT_IN_WIDTH, AT_IN_HEIGHT, 1, PI_BUFFER_FORMAT_GRAY);
...
/*-------------------reading input data-----------------------------*/
	...
	while(1){
        ...
        // crop [AT_IN_HEIGHT x AT_IN_WIDTH]
        int ps=0;
        for(int i =0;i<CAMERA_HEIGHT;i++){
        	for(int j=0;j<CAMERA_WIDTH;j++){
        		if (i<AT_IN_HEIGHT && j<AT_IN_WIDTH){
		...
		if (ReadImageFromFile(ImageName, AT_IN_WIDTH, AT_IN_HEIGHT, 1, imgin_unsigned, AT_IN_SIZE*sizeof(unsigned char), 0, 0)) {
  	...
  	#ifdef HAVE_LCD
  		pi_display_write(&display, &buffer, 0, 0, AT_IN_WIDTH, AT_IN_HEIGHT);
  	#endif 
